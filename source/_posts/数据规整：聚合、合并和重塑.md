...
title: 数据规整：聚合、合并和重塑
categories: Python
date: 2023-08-16 13:25:00
---

在许多应用中，数据可能分散在许多文件或数据库中，存储的形式也不利于分析。本章关注可以聚合、合并、重塑数据的方法。

首先介绍pandas的层次化索引，它广泛用于以上操作。然后，我深入介绍了一些特殊的数据操作。
...
<img src="http://upload-images.jianshu.io/upload_images/7178691-e49b3341f4a3c90e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240" alt="表8-1 不同的连接类型" style="zoom:50%;" />
...
<img src="http://upload-images.jianshu.io/upload_images/7178691-35ca716a4f1b8475.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240" style="zoom:50%;" />

<img src="http://upload-images.jianshu.io/upload_images/7178691-c86672e733ceccd9.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240" style="zoom:50%;" />
...
<img src="http://upload-images.jianshu.io/upload_images/7178691-339436563b519415.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240" alt="表8-3 concat函数的参数" style="zoom:50%;" />
...
In [113]: np.where(pd.isnull(a), b, a) # 表示一种等价于面向数组的if-else
...
In [141]: periods = pd.PeriodIndex(year=data.year, quarter=data.quarter,
   .....:                          name='date')
	# 创建了一个PeriodIndex对象，用来表示时间序列的索引。这里使用了year和quarter两列的数据，并将其命名为date。
In [142]: columns = pd.Index(['realgdp', 'infl', 'unemp'], name='item')
	# 创建了一个Index对象，其中包含了需要保留的列名，即'realgdp'、'infl'和'unemp'，并将其命名为item。
In [143]: data = data.reindex(columns=columns)
	# 重新索引了数据集的列，只保留了columns中指定的列。
In [144]: data.index = periods.to_timestamp('D', 'end')
	# 将数据集的索引修改为periods.to_timestamp('D', 'end')，即将时间序列的索引修改为每个季度末的日期。
In [145]: ldata = data.stack().reset_index().rename(columns={0: 'value'})
	# 将数据集进行了重构和重塑，将原来的列转换为行，并将结果存储在ldata中。同时，通过rename()函数将新的列命名为'value'。
```
...
In [147]: pivoted = ldata.pivot('date', 'item', 'value')
	# 将之前重构和重塑后的数据集ldata进行透视操作，将date列作为行索引，item列作为列索引，value列中的值作为数据填充到新的数据集中。
In [148]: pivoted
Out[148]: 
