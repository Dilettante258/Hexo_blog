...
虽然pandas采用了大量的NumPy编码风格，但<u>二者最大的不同是pandas是专门为处理表格和混杂数据设计的。而NumPy更适合处理统一的数值数组数据。</u>
...
<u>Series的字符串表现形式为：索引在左边，值在右边。由于我们没有为数据指定索引，于是会自动创建一个0到N-1（N为数据的长度）的整数型索引。</u>你可以通过Series 的values和index属性获取其数组表示形式和索引对象：
...
在这个例子中，<u>sdata中跟states索引相匹配的那3个值会被找出来并放到相应的位置上，但由于"California"所对应的sdata值找不到，所以其结果就为NaN（即“非数字”（not a number），在pandas中，它用于表示缺失或NA值）。因为‘Utah’不在states中，它被从结果中除去。</u>
...
对于许多应用而言，<u>Series最重要的一个功能是，它会根据运算的索引标签自动对齐数据</u>：
```python
In [35]: obj3
Out[35]: 
Ohio      35000
...
<u>如果指定了列序列，则DataFrame的列就会按照指定顺序进行排列</u>：
...
frame2[column]适用于任何列的名，但是<u>frame2.column只有在列名是一个合理的Python变量名时才适用</u>。
...
| ismonotonic  | 当各元素均大于等于前一个元素时，返回True           |
...
**对于时间序列这样的有序数据，重新索引时可能需要做一些插值处理。method选项即可达到此目的，例如，使用ffill可以实现*前向*值填充**：
...
| df.at[label_i, label j]  | 通过行和列标签，选取单一的标量                               |
| df.iat[i, j]             | 通过行和列的位置(整数)，选取单一的标量                       |
| reindex                  | 通过标签选取行或列                                           |
| get_value,set value      | 通过行和列标签选取单一值                                     |
...
使用df1的add方法，传入df2以及**一个fill_value参数**：
```python
...
## 排序和排名
**根据条件对数据集排序（sorting）也是一种重要的内置运算。要对行或列索引进行排序（按字典顺序），可使用sort_index方法，它将*返回*一个已排序的新对象**：
...
排名会从1开始一直到数组中有效数据的数量。接下来介绍**Series和DataFrame的rank方法。默认情况下，rank是通过“为各组分配一个平均排名”的方式破坏平级关系的**：
```python
...
| dense   | 类似于'min方法，但是排名总是在组间增加1，而不是组中相同的元素数 |
...
# 汇总和计算描述统计
pandas对象拥有一组常用的数学和统计方法。它们大部分都属于约简和汇总统计，用于从Series中提取单个值（如sum或mean）或从DataFrame的行或列中提取一个Series。跟对应的NumPy数组方法相比，它们都是<u>基于没有缺失数据的假设而构建的</u>。看一个简单的DataFrame：
```python
